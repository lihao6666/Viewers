...
const ViewportActionBar = ({
  ...
  const studyDateClasses = () =>
    `text-aqua-pale ${showSeriesDesc ? '' : `mr-1 ${textEllipsisClasses}`}`;
  ...
  const resizeCallback = useCallback(() => {
    ...
    if (!studyDateElemBBox || !showPatientInfoElemBBox) {
      return;
    }
  ...
  return (
    ...
    >
      {!!label?.length && (
        <span className="mr-2 font-bold text-lg text-aqua-pale text-large">{label}</span>
      )}
      {getStatusComponent()}
      <span
        ref={seriesDescElemRef}
        className={`ml-2 text-aqua-pale ${textEllipsisClasses}`}
      >
        {seriesDescription}
      </span>
      ...
      <span ref={studyDateElemRef} className={studyDateClasses()}>
        {patientName}/{formatSex(patientSex)}/{patientAge}
      </span>
      {showArrows && (
        <>
          <Icon
